...
from .utils import make_request, normalize_url, get_request_client
...
class Crawler:
    ...
    async def fetch_url(self, url: str, **kwargs) -> httpx.Response | None:
        """
        Fetch a URL asynchronously using the configured client and semaphore
        """
        return await make_request(
            url, self.client, self.semaphore, logger=self.logger, **kwargs
        )

    def format_search_url(self) -> str:
        """
        Format the GitHub search URL based on keywords and search type
        """
        query = " ".join(self.keywords)
        params = urlencode({"q": query, "type": self.search_type})
        return normalize_url("search?" + params)
    ...
    async def run(self) -> list[dict] | None:
        ...
        try:
            ...
            search_data = await self.fetch_url(search_url)
